...
#define FILT_LEN         8u
#define ALARM_TICKS     250u    // 5 sec
#define CALM_TICKS      150u  // 3 sec
#define BLINK_TICKS     10u 
#define BEEP_TICKS      2u 
...
static void uart_log(uint16_t pp, uint16_t avg, NoiseState st, uint16_t warn, uint16_t alarm_t, uint8_t pidx);
...
static void lcd_update(uint16_t avg, NoiseState st, uint16_t alarm_t, uint8_t pidx);
/* USER CODE END PFP */
